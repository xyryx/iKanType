...
public class KBView: UIView {
    let NUMBER_OF_ROWS:NSInteger = 4;
    let NUMBER_OF_KEYS_PER_ROW:NSInteger = 10;
    ...
    func baseInit() {
        ...
        self.numberOfKeysPerRow = NUMBER_OF_KEYS_PER_ROW
    ...
    func addKeyButtons() {
        for i in 0 ..< self.numberOfRows {
            ...
            for j in 0 ..< self.numberOfKeysPerRow {
                if i * self.numberOfKeysPerRow + j < self.keyboard!.alpha!.count {
                    let button: KBButton = KBButton()
                    button.translatesAutoresizingMaskIntoConstraints = false
                    button.isUserInteractionEnabled = false
                    button.sizeToFit()
                    let key: KeyModel = self.keyboard!.alpha![i * self.numberOfKeysPerRow + j] as! KeyModel
    ...
    func setupConstraints() {
        ...
        // top constraint
        let row: UIView = rows[rows.count - 3]
        ...
        // top constraint
        let lastRow: UIView = rows[rows.count - 2]
        ...
        for i in 0 ..< rows.count {
            ...
            var rightAlignmentAttribute: NSLayoutAttribute
            if i == rows.count - 2 {
            ...
            if (previousRow == nil) {
                self.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|-0-[row]", options: NSLayoutFormatOptions(rawValue: 0), metrics: nil, views: views ))
            }
            else {
                self.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:[previousRow]-0-[row]", options: NSLayoutFormatOptions(rawValue: 0), metrics: nil, views: views ))
            }
    ...
    func resetShiftKey(button: UIButton) {
    ...
    func handleDeleteLongGesture(longPressRecognizer: UILongPressGestureRecognizer) {
    ...
    func setReturnKeyTitle() {
        ...
        returnKeyText = keyboard!.getKeyboardTitle(returnKeyType)
